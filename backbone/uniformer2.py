...
class VisionTransformer(nn.Module):
    def __init__(
        self, 
        # backbone
        input_resolution, patch_size, width, layers, heads, backbone_drop_path_rate=0.,
...
def uniformerv2_b16(model_path = None,
    pretrained=True, use_checkpoint=False, checkpoint_num=[0],
    t_size=8, dw_reduction=1.5, backbone_drop_path_rate=0., 
    ...
    if pretrained:
        ...
        ww = {}
        for k in state_dict:
            ww[k.replace('backbone.', '')] = state_dict[k]
        load_state_dict(model, ww)
    return model.eval()
...
def uniformerv2_l14(
    ...
    '''
    input shape: (B, C, T, H, W) `batch_size` x  `channel` x `num frames` x `height` x `width`
    output shape: (1, B, 1024) `1` x `batch_size` x `1024`
    '''
    model = VisionTransformer(
        input_resolution=224,
        patch_size=14,
...
def uniformerv2_l14_336(model_path = None,
    pretrained=True, use_checkpoint=False, checkpoint_num=[0],
    t_size=16, dw_reduction=1.5, backbone_drop_path_rate=0., 
    temporal_downsample=True,
    no_lmhra=False, double_lmhra=True,
